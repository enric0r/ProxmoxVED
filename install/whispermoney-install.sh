...
sed -i "s|^EMAIL_VERIFICATION_ENABLED=.*|EMAIL_VERIFICATION_ENABLED=true|" .env
...
EOF
systemctl enable -q --now php${PHP_VER}-fpm redis-server caddy whispermoney-ssr whispermoney-queue whispermoney-emails whispermoney-scheduler.timer
